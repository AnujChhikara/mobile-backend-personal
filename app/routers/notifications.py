...
async def send_expo_notification(expo_token: str, title: str, body: str, data: Optional[dict] = None) -> dict:
...
async def send_notification_to_all(notification: NotificationRequest = NotificationRequest()):
    """
    ...
    if not tokens:
        raise HTTPException(
            ...
            detail="No expo tokens found in database"
        )
...
async def send_test_notification():
    ...
    if not tokens:
        return {
            "success": False,
            "message": "No expo tokens found in database",
            "total_tokens": 0
        }
    ...
    for token_doc in tokens:
        try:
            ...
            print(f"💥 Exception for user: {token_doc['user_id']}, error: {str(e)}")
            failed += 1
...
async def send_notification_to_user(user_id: str, notification: NotificationRequest = NotificationRequest()):
