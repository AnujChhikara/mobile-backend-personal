...
async def create_expo_token(token_data: ExpoTokenCreate):
    ...
    if existing:
        ...
        await collection.update_one(
            ...
            {"$set": {"expo_token": token_data.expo_token, "updated_at": now}},
        )
...
async def get_expo_token(token_id: str):
    ...
    if not ObjectId.is_valid(token_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid token ID format"
        )

    token = await collection.find_one({"_id": ObjectId(token_id)})

    if not token:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expo token not found")
...
async def update_expo_token(token_id: str, token_data: ExpoTokenUpdate):
    ...
    await collection.update_one({"_id": ObjectId(token_id)}, {"$set": update_data})
